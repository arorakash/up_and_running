...
def handler(config, common):
    try:
        ...
        for behaviour in config['behaviours']:
            behaviour_target = behaviour['behaviour_target']
            behaviour_key = behaviour['behaviour_key']
            behaviour_value = behaviour['behaviour_value']

            subprocess_command = f"gsettings set {behaviour_target} {behaviour_key} '{behaviour_value}'"
